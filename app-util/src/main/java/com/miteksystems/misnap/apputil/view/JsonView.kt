...
class JsonView : LinearLayout {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        try {
            textColorString = attributes.getColor(
                R.styleable.JsonViewer_textColorString,
                ContextCompat.getColor(context, R.color.misnapAppUtilJsonViewerTextColorString)
            )
            textColorNumber = attributes.getColor(
                R.styleable.JsonViewer_textColorNumber,
                ContextCompat.getColor(context, R.color.misnapAppUtilJsonViewerTextColorNumber)
            )
            textColorBool = attributes.getColor(
                R.styleable.JsonViewer_textColorBool,
                ContextCompat.getColor(context, R.color.misnapAppUtilJsonViewerTextColorBool)
            )
            textColorNull = attributes.getColor(
                R.styleable.JsonViewer_textColorNull,
                ContextCompat.getColor(context, R.color.misnapAppUtilJsonViewerTextColorNull)
            )
            textSize = attributes.getDimension(
                R.styleable.JsonViewer_textSize,
                resources.getDimension(R.dimen.misnapAppUtilJsonViewerTextSize))
        } finally {
            attributes.recycle()
        }
    }
    ...
    fun collapseJson() {
        ...
        while (i < this.childCount) {
            if (this.getChildAt(i) is TextView &&
                ...
                changeVisibility(this.getChildAt(i + 1) as ViewGroup, VISIBLE)
                i += 2
            }
            i++
        }
            ...
            if (!childDisplayed) {
                ...
                if (jsonNode is JSONArray) {
                    ...
                    if (jsonNode is String) {
                        append("\"" + jsonNode + "\"",
                            ForegroundColorSpan(textColorString), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
